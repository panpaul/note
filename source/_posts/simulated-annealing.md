...
题目就不复述了，我们由题意可知当小球总体的重力势能最小时达到平衡点。

在本题中有几点需要注意的：

1. 下一个随机点是基于上一个点随机生成的。
2. 为提高正确率，可以多次进行模拟退火。
...
long double tmpX, tmpY, tX = 0, tY = 0;
long double minE, tE;

default_random_engine randomEngine(time(nullptr));
uniform_real_distribution<long double> u(-1000, 1000);
uniform_real_distribution<long double> uu(0, 10000);

inline long double energy()
{// 计算“重力势能”
    long double r = 0;
    for (register int i = 0; i < n; i++)
    {
        r += sqrt((tX - (long double)points[i].x) * (tX - (long double)points[i].x) + (tY - (long double)points[i].y) * (tY - (long double)points[i].y)) * points[i].w;
    }
    return r;
}

inline void thsub(long double temp)
{// 概率接受的判断
    tX = tmpX + u(randomEngine) * temp;
    tY = tmpY + u(randomEngine) * temp;
    tE = energy();
    long double delta = tE - minE;
    if (delta < 0)
    {
        tmpX = tX;
        tmpY = tY;
    ...
    long double tmp1 = exp(-delta / temp) * 10000;
    long double tmp2 = uu(randomEngine);
    if (tmp1 > tmp2)
    {
        tmpX = tX;
        tmpY = tY;
...
int main()
{
    ...
    tX = tmpX = (double)sumX / n;
    tY = tmpY = (double)sumY / n;
    minE = energy();
    th();
    th();
    th();// 多次退火提高精确度
    printf("%.3Lf %.3Lf", tmpX, tmpY);
    return 0;
}
```
